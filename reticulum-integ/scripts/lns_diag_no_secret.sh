...
#
# Verifies that `lnstest diag` never emits the raw transport-identity bytes
...
# Lowercase hex of the 64 identity bytes, the way `lnstest diag` would render
...
lnstest diag -c /root/.reticulum > /tmp/lns-diag-out.txt
if grep -qiF "$h" /tmp/lns-diag-out.txt; then
    echo "FAIL: identity bytes leaked into lnstest diag output" >&2
    exit 1
fi
