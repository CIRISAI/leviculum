...
//! (reticulum-nrf/src/lib.rs). A first-fit allocator can FRAGMENT under churny
...
//!
//! Run: `cargo test -p reticulum-core --test heap_fragmentation -- --nocapture`
...
const HEAP_SIZE: usize = 64 * 1024; // matches reticulum-nrf HEAP_SIZE
        ...
        None => {
            ...
            println!(
                "[heap_frag] NO EXHAUSTION: the 64 KiB LlffHeap absorbed all {} ops \
                 ...
                 (reticulum-nrf lora/usb/ble per-frame buffers), not the shared core.",
                r.ops,
                r.peak_used,
                100.0 * r.peak_used as f64 / HEAP_SIZE as f64,
                r.min_largest_block,
