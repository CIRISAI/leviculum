...
pub struct EmbeddedStorage {
    ...
    ///
    /// **Insert paths:**
    /// - `transport.rs:1031`: `create_receipt`: fresh-key (truncated
    ///   hash of the just-sent packet).
    /// - `transport.rs:1051`: `create_receipt_with_timeout`: fresh-key
    ///   (same shape as `create_receipt`).
    /// - `transport.rs:1071`: `mark_receipt_delivered`: re-insert
