...
pub struct EmbeddedStorage {
    ...
    ///
    /// **Insert paths:**
    /// - `transport.rs:908`: `create_receipt`: fresh-key (truncated
    ///   hash of the just-sent packet).
    /// - `transport.rs:928`: `create_receipt_with_timeout`: fresh-key
    ///   (same shape as `create_receipt`).
    /// - `transport.rs:948`: `mark_receipt_delivered`: re-insert
