...
lint-nrf:
    ...
    # leviculum-gnss-presence, leviculum-gnss-init and
    # leviculum-telemetry-policy are the pure, host-testable crates inside
    # the leviculum-nrf workspace: clippy + tests run on the host triple
    # (the workspace's .cargo/config defaults to thumbv7em).
    cd leviculum-nrf && cargo clippy -p leviculum-screen -p leviculum-sd-policy -p leviculum-gnss-time -p leviculum-gnss-presence -p leviculum-gnss-init -p leviculum-telemetry-policy --target $(rustc -vV | sed -n 's/host: //p') -- -D warnings
    cd leviculum-nrf && cargo test -p leviculum-screen -p leviculum-sd-policy -p leviculum-gnss-time -p leviculum-gnss-presence -p leviculum-gnss-init -p leviculum-telemetry-policy --target $(rustc -vV | sed -n 's/host: //p')
