    ...
    pub(crate) fn parse(
        ...
        match (coord, hemi) {
            ...
            (Some(lat), Some(lat_hemi)) => {
                ...
                let min_sec =
                    crate::fixed::f64_from_str(&lat[2..]).ok_or("Wrong latitude field format")?;
                let minutes = min_sec as u8;
                let seconds = ((min_sec - minutes as f64) * 60f64) as f32;
                Ok(Some(Latitude {
                    degrees,
    ...
    pub(crate) fn parse(
        ...
        match (coord, hemi) {
            ...
            (Some(lon), Some(lon_hemi)) => {
                ...
                let min_sec =
                    crate::fixed::f64_from_str(&lon[3..]).ok_or("Wrong longitude field format")?;
                let minutes = min_sec as u8;
                let seconds = ((min_sec - minutes as f64) * 60f64) as f32;
                Ok(Some(Longitude {
                    degrees,
    ...
    pub(crate) fn parse(input: Option<&str>) -> Result<Option<Self>, &'static str> {
        match input {
            ...
            Some(alt) => Ok(Some(Altitude {
                meters: crate::fixed::f32_from_str(alt).ok_or("Wrong altitude field format")?,
            })),
            _ => Ok(None),
        }
    }
...
impl Speed {
    ...
    pub(crate) fn parse(input: Option<&str>) -> Result<Option<Self>, &'static str> {
        match input {
            ...
            Some(speed) => crate::fixed::f32_from_str(speed)
                .ok_or("Wrong speed field format")
...
impl Course {
    pub(crate) fn parse(input: Option<&str>) -> Result<Option<Self>, &'static str> {
        match input {
            ...
            Some(course) => crate::fixed::f32_from_str(course)
                .ok_or("Wrong course field format")
    ...
    pub(crate) fn parse_from_mvar_mdir(
        ...
        if let (Some(course), Some(variation), Some(direction)) = (true_course, mvar, mdir) {
            if variation.len() == 0 && direction.len() == 0 {
                ...
                let magnetic = crate::fixed::f32_from_str(variation)
                    .ok_or("Wrong magnetic variation field format!")?;
                match direction {
