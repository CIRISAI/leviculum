...
as `false` (`ini_config.rs:716-727`).
...
| `share_instance` | bool | `false` | Listen on the abstract Unix socket `\0rns/<instance_name>` for local clients. Required for `lnstest diag`, `rnstatus`, Sideband etc. to attach. (`config.rs:37-40`, `205`; key `share_instance` → `shared_instance`, `ini_config.rs:263-265`) |
...
| `storage_path` | path | unset | Where identity, known destinations and packet hashlist live. Relative values resolve against the config dir. (`config.rs:97-98`; `storage_path` (`ini_config.rs:366`)) |
...
only; they have no INI key in `apply_reticulum_key` (`ini_config.rs:233-346`)
...
(`ini_config.rs:681-711`)). An interface of any other type is skipped
...
(`ini_config.rs:376-561`); the struct they land in, with its defaults,
is `InterfaceConfig` (`config.rs:228-505`).

### Keys common to every interface

| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `type` | string | (required) | Interface type, one of the eleven above. (`type` (`ini_config.rs:428`)) |
| `enabled` | bool | `true` | Bring this interface up; the legacy spelling `interface_enabled` is honoured too. (`enabled` (`ini_config.rs:429`); `InterfaceConfig::enabled` (`config.rs:239-241`)) |
| `outgoing` | bool | `true` | Allow sending outgoing packets. (`outgoing` (`ini_config.rs:433`); `InterfaceConfig::outgoing` (`config.rs:283-285`)) |
| `bitrate` | u64 (bps) | per type | Override the interface's own bitrate figure, which feeds announce bandwidth capping and timing. Values below `MINIMUM_BITRATE` (`constants.rs:324-327`), 5 bps, are ignored. (`bitrate` (`ini_config.rs:486-492`); `InterfaceConfig::bitrate` (`config.rs:286-291`)) |
| `buffer_size` | usize | per type | Channel buffer size. (`buffer_size` (`ini_config.rs:537`); `InterfaceConfig::buffer_size` (`config.rs:424-426`)) |

### TCP server (`TCPServerInterface`)

| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `listen_ip` | string | unset | Address to bind. (`listen_ip` (`ini_config.rs:440`)) |
| `listen_port` | u16 | unset | Port to listen on. (`listen_port` (`ini_config.rs:441`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `target_host` | string | unset | Remote host to connect to. (`target_host` (`ini_config.rs:442`)) |
| `target_port` | u16 | unset | Remote port. (`target_port` (`ini_config.rs:443`)) |
| `reconnect_interval` | u64 (sec) | `5` | Delay between reconnect attempts. (`reconnect_interval` (`ini_config.rs:538`); `InterfaceConfig::reconnect_interval_secs` (`config.rs:426-427`)) |
| `max_reconnect_tries` | u64 | unlimited | Give up after this many attempts; unset means never. (`max_reconnect_tries` (`ini_config.rs:539`); `InterfaceConfig::max_reconnect_tries` (`config.rs:428-429`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `listen_ip` | string | `0.0.0.0` | Local bind address. (`listen_ip` (`ini_config.rs:440`)) |
| `listen_port` | u16 | unset | Local bind port. (`listen_port` (`ini_config.rs:441`)) |
| `forward_ip` | string | unset | Broadcast/forward address or hostname. Names are resolved at runtime and re-resolved periodically; a resolution failure is a logged interface error, not a config error. (`forward_ip` (`ini_config.rs:451`); `InterfaceConfig::forward_ip` (`config.rs:363-373`)) |
| `forward_port` | u16 | unset | Broadcast/forward port. (`forward_port` (`ini_config.rs:452`)) |
| `port` | u16 | unset | Fills both `listen_port` and `forward_port`; either explicit key wins over it. (`port` (`ini_config.rs:441`)) |
| `device` | string | unset | Kernel interface name; its IPv4 broadcast address fills both `listen_ip` and `forward_ip`. Either explicit key wins over it. (`device` (`ini_config.rs:438`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `group_id` | string | unset | Multicast group identifier; isolate co-located meshes by setting different IDs. (`group_id` (`ini_config.rs:551`); `InterfaceConfig::group_id` (`config.rs:432-434`)) |
| `discovery_scope` | string | unset | Multicast scope: `link`, `admin`, `site`, `organisation`, `global`. (`discovery_scope` (`ini_config.rs:552`); `InterfaceConfig::discovery_scope` (`config.rs:434-435`)) |
| `discovery_port` | u16 | `29716` | Discovery (announce) port. (`discovery_port` (`ini_config.rs:553`); `InterfaceConfig::discovery_port` (`config.rs:436-437`)) |
| `data_port` | u16 | `42671` | Data port. (`data_port` (`ini_config.rs:554`); `InterfaceConfig::data_port` (`config.rs:438-439`)) |
| `devices` | string (CSV) | unset | Whitelist of NIC names to use. (`devices` (`ini_config.rs:555`); `InterfaceConfig::devices` (`config.rs:440-441`)) |
| `ignored_devices` | string (CSV) | unset | Blacklist of NIC names to skip. (`ignored_devices` (`ini_config.rs:556`); `InterfaceConfig::ignored_devices` (`config.rs:442-443`)) |
| `multicast_loopback` | bool | unset (inherits `true`) | Multicast loopback (`IPV6_MULTICAST_LOOP`), the carrier self-echo mechanism. Unset inherits the default `true`, matching Python-RNS; set `no` to opt out. (`multicast_loopback` (`ini_config.rs:557`); `InterfaceConfig::multicast_loopback` (`config.rs:446-449`)) |
| `multicast_address_type` | string | unset (inherits `temporary`) | Multicast address type of the discovery group, `temporary` or `permanent`. It is part of the group address, so peers must agree on it: an `lnsd` node left on the default next to a `permanent`-type `rnsd` peer group discovers nobody, and nothing on either side says why. Unset inherits `temporary`, the group Python joins when the key is absent. A value that is neither spelling is refused at startup rather than resolved to `temporary` the way Python resolves it. (`multicast_address_type` (`ini_config.rs:559-565`); `InterfaceConfig::multicast_address_type` (`config.rs:448-453`); `MulticastAddressType` (`interfaces/auto_interface/mod.rs:42-91`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `device` | string | default adapter | BlueZ adapter to use, e.g. `hci0`. (`device` (`ini_config.rs:438`)) |
| `max_connections` | usize | `4` | Simultaneous BLE link cap, both GATT roles counted together. The default is the firmware's `MAX_LINKS` (4), not the reference's 7: 3-4 links is the protocol's reliable ceiling. (`max_connections` (`ini_config.rs:623`); `InterfaceConfig::max_connections` (`config.rs:567-570`)) |
| `min_rssi` | i16 (dBm) | `-85` | Sightings weaker than this are not dialled. (`min_rssi` (`ini_config.rs:624`); `InterfaceConfig::min_rssi` (`config.rs:571-573`)) |
| `discovery_interval` | f64 (sec) | `5` | Pause between the 2-second BLE scan windows. (`discovery_interval` (`ini_config.rs:625`); `InterfaceConfig::discovery_interval` (`config.rs:574-576`)) |
| `enable_central` | bool | `true` | Run the scanning + dialling central role. (`enable_central` (`ini_config.rs:626`); `InterfaceConfig::enable_central` (`config.rs:577-579`)) |
| `enable_peripheral` | bool | `true` | Run the advertising + GATT-server peripheral role. Disabling both roles is a config error. (`enable_peripheral` (`ini_config.rs:627`); `InterfaceConfig::enable_peripheral` (`config.rs:580-582`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `port` | string | unset | Serial device path, e.g. `/dev/ttyACM0`. (`port` (`ini_config.rs:441`); `InterfaceConfig::port` (`config.rs:479-480`)) |
| `speed` / `baudrate` | u32 | unset | Serial baud rate (either spelling). (`speed` (`ini_config.rs:454`); `InterfaceConfig::speed` (`config.rs:380-381`)) |
| `databits` | u8 | unset | Data bits. (`databits` (`ini_config.rs:455`); `InterfaceConfig::databits` (`config.rs:382-383`)) |
| `parity` | string | unset | `none`, `even`, or `odd`. (`parity` (`ini_config.rs:456`); `InterfaceConfig::parity` (`config.rs:384-385`)) |
| `stopbits` | u8 | unset | Stop bits. (`stopbits` (`ini_config.rs:457`); `InterfaceConfig::stopbits` (`config.rs:386-387`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `frequency` | u64 (Hz) | unset | LoRa centre frequency. (`frequency` (`ini_config.rs:540`); `InterfaceConfig::frequency` (`config.rs:487-489`)) |
| `bandwidth` | u32 (Hz) | unset | LoRa bandwidth. (`bandwidth` (`ini_config.rs:492`); `InterfaceConfig::bandwidth` (`config.rs:466-467`)) |
| `spreadingfactor` / `spreading_factor` | u8 | unset | LoRa spreading factor (either spelling). (`spreadingfactor` (`ini_config.rs:542`); `InterfaceConfig::spreading_factor` (`config.rs:508-509`)) |
| `codingrate` / `coding_rate` | u8 | unset | LoRa coding rate (either spelling). (`codingrate` (`ini_config.rs:543`); `InterfaceConfig::coding_rate` (`config.rs:510-511`)) |
| `txpower` / `tx_power` | i8 (dBm) | unset (resolves to the board maximum, 22 dBm) | Transmit power (either spelling). Unset asks the board for its maximum — a board that can do less clamps and says so — rather than the 0 dBm (1 mW) Python-Reticulum resolves it to, which has no symptom at the node. An explicit `txpower = 0` still means 0. Above roughly 7 dBi of antenna gain, 22 dBm conducted exceeds the EU 27 dBm ERP allowance and has to be set down. (`txpower` (`ini_config.rs:544`); `InterfaceConfig::tx_power` (`config.rs:512-513`); `resolve_tx_power` (`rnode.rs:702`); [deviation](../concepts/python-rns-compatibility.md)) |
| `flow_control` | bool | unset | Wait for the RNode's `CMD_READY` before the next TX. (`flow_control` (`ini_config.rs:565`); `InterfaceConfig::flow_control` (`config.rs:530-531`)) |
| `airtime_limit_short` | f64 (%) | unset | Short-term airtime cap, percent (0.0–100.0). (`airtime_limit_short` (`ini_config.rs:566`); `InterfaceConfig::airtime_limit_short` (`config.rs:532-533`)) |
| `airtime_limit_long` | f64 (%) | unset | Long-term airtime cap, percent (0.0–100.0). (`airtime_limit_long` (`ini_config.rs:567`); `InterfaceConfig::airtime_limit_long` (`config.rs:534-535`)) |
| `csma_enabled` | bool | unset | Carried in the LNode radio-config frame and reported back, but current firmware no longer obeys it: LoRa channel access (pre-TX jitter plus CAD listen-before-talk) is always on, matching the RNode firmware, which offers no CSMA disable either. Only firmware older than the change still honours the flag. (`csma_enabled` (`ini_config.rs:568`); `InterfaceConfig::csma_enabled` (`config.rs:536-537`)) |
| `preamble_symbols` | u16 (symbols) | unset (derived from the PHY) | LoRa preamble length pushed to LNode firmware in the radio-config frame (`SerialInterface` only). Unset derives what an RNode peer programs for the same PHY — 24 symbols at SF7/BW125, the 18-symbol floor from SF8 down — so a mixed pair agrees on the wire; set it only to pin a value against a non-conforming peer. A pin above roughly 20 symbols / 164 ms on air is warned about at startup and not refused: SX127x receivers (every RNode) were measured going deaf above that, losing every frame from the interface silently and one-way, while an SX126x peer copes ([Codeberg #315](https://codeberg.org/Lew_Palm/leviculum/issues/315)). Not the same key as the KISS `preamble` (TX delay in ms), which never reaches a LoRa modem. (`preamble_symbols` (`ini_config.rs:549`); `InterfaceConfig::preamble_symbols` (`config.rs:516-528`); `derive_preamble_symbols` (`rnode.rs:858`); `preamble_ceiling_warning` (`interfaces/serial.rs`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `networkname` / `network_name` | string | unset | Network name for IFAC (either spelling). (`networkname` (`ini_config.rs:602`); `InterfaceConfig::networkname` (`config.rs:496-498`)) |
| `passphrase` / `pass_phrase` | string | unset | IFAC passphrase (either spelling). (`passphrase` (`ini_config.rs:514`); `InterfaceConfig::passphrase` (`config.rs:498-499`)) |
| `ifac_size` | usize (bits) | unset | IFAC size, specified in bits in the file and stored as bytes (`bits / 8`). Values below 8 bits are dropped, as Python drops them, so the interface falls back to its per-type default. (`ifac_size` (`ini_config.rs:604-610`); `InterfaceConfig::ifac_size` (`config.rs:500-501`)) |
