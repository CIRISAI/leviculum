...
as `false` (`ini_config.rs:819-830`).
...
| `link_table_cap` | usize | profile | Maximum `link_table` entries — links this node routes for, not its own (that is `max_links`). Desktop `8192`, compact `1024`. (`config.rs:222-229`; `ini_config.rs:403-405`) |
| `announce_table_cap` | usize | profile | Maximum `announce_table` entries, the pending-rebroadcast queue. Each holds a full copy of an announce packet. Desktop `16384`, compact `2048`. (`config.rs:230-236`; `ini_config.rs:406-408`) |
| `destination_cap` | usize | profile | Maximum entries in the destination-keyed tables: `announce_cache`, `announce_rate_table`, `known_ratchets`, `known_dest_use`. Desktop `50000`, compact `4096`. One key for four tables because they share one population. (`config.rs:237-247`; `ini_config.rs:409-411`) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `type` | string | (required) | Interface type, one of the eleven above. (`type` (`ini_config.rs:515`)) |
| `enabled` | bool | `true` | Bring this interface up; the legacy spelling `interface_enabled` is honoured too. (`enabled` (`ini_config.rs:516`); `InterfaceConfig::enabled` (`config.rs:334-336`)) |
| `outgoing` | bool | `true` | Allow sending outgoing packets. (`outgoing` (`ini_config.rs:520`); `InterfaceConfig::outgoing` (`config.rs:385-387`)) |
| `bitrate` | u64 (bps) | per type | Override the interface's own bitrate figure, which feeds announce bandwidth capping and timing. Values below `MINIMUM_BITRATE` (`constants.rs:324-327`), 5 bps, are ignored. (`bitrate` (`ini_config.rs:573-579`); `InterfaceConfig::bitrate` (`config.rs:388-393`)) |
| `buffer_size` | usize | per type | Channel buffer size. (`buffer_size` (`ini_config.rs:624`); `InterfaceConfig::buffer_size` (`config.rs:526-528`)) |

### TCP server (`TCPServerInterface`)

| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `listen_ip` | string | unset | Address to bind. (`listen_ip` (`ini_config.rs:527`)) |
| `listen_port` | u16 | unset | Port to listen on. (`listen_port` (`ini_config.rs:528`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `target_host` | string | unset | Remote host to connect to. (`target_host` (`ini_config.rs:529`)) |
| `target_port` | u16 | unset | Remote port. (`target_port` (`ini_config.rs:530`)) |
| `reconnect_interval` | u64 (sec) | `5` | Delay between reconnect attempts. (`reconnect_interval` (`ini_config.rs:625`); `InterfaceConfig::reconnect_interval_secs` (`config.rs:528-529`)) |
| `max_reconnect_tries` | u64 | unlimited | Give up after this many attempts; unset means never. (`max_reconnect_tries` (`ini_config.rs:626`); `InterfaceConfig::max_reconnect_tries` (`config.rs:530-531`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `listen_ip` | string | `0.0.0.0` | Local bind address. (`listen_ip` (`ini_config.rs:527`)) |
| `listen_port` | u16 | unset | Local bind port. (`listen_port` (`ini_config.rs:528`)) |
| `forward_ip` | string | unset | Broadcast/forward address or hostname. Names are resolved at runtime and re-resolved periodically; a resolution failure is a logged interface error, not a config error. (`forward_ip` (`ini_config.rs:538`); `InterfaceConfig::forward_ip` (`config.rs:465-475`)) |
| `forward_port` | u16 | unset | Broadcast/forward port. (`forward_port` (`ini_config.rs:539`)) |
| `port` | u16 | unset | Fills both `listen_port` and `forward_port`; either explicit key wins over it. (`port` (`ini_config.rs:528`)) |
| `device` | string | unset | Kernel interface name; its IPv4 broadcast address fills both `listen_ip` and `forward_ip`. Either explicit key wins over it. (`device` (`ini_config.rs:525`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `group_id` | string | unset | Multicast group identifier; isolate co-located meshes by setting different IDs. (`group_id` (`ini_config.rs:638`); `InterfaceConfig::group_id` (`config.rs:534-536`)) |
| `discovery_scope` | string | unset | Multicast scope: `link`, `admin`, `site`, `organisation`, `global`. (`discovery_scope` (`ini_config.rs:639`); `InterfaceConfig::discovery_scope` (`config.rs:536-537`)) |
| `discovery_port` | u16 | `29716` | Discovery (announce) port. (`discovery_port` (`ini_config.rs:640`); `InterfaceConfig::discovery_port` (`config.rs:538-539`)) |
| `data_port` | u16 | `42671` | Data port. (`data_port` (`ini_config.rs:641`); `InterfaceConfig::data_port` (`config.rs:540-541`)) |
| `devices` | string (CSV) | unset | Whitelist of NIC names to use. (`devices` (`ini_config.rs:642`); `InterfaceConfig::devices` (`config.rs:542-543`)) |
| `ignored_devices` | string (CSV) | unset | Blacklist of NIC names to skip. (`ignored_devices` (`ini_config.rs:643`); `InterfaceConfig::ignored_devices` (`config.rs:544-545`)) |
| `multicast_loopback` | bool | unset (inherits `true`) | Multicast loopback (`IPV6_MULTICAST_LOOP`), the carrier self-echo mechanism. Unset inherits the default `true`, matching Python-RNS; set `no` to opt out. (`multicast_loopback` (`ini_config.rs:644`); `InterfaceConfig::multicast_loopback` (`config.rs:548-551`)) |
| `multicast_address_type` | string | unset (inherits `temporary`) | Multicast address type of the discovery group, `temporary` or `permanent`. It is part of the group address, so peers must agree on it: an `lnsd` node left on the default next to a `permanent`-type `rnsd` peer group discovers nobody, and nothing on either side says why. Unset inherits `temporary`, the group Python joins when the key is absent. A value that is neither spelling is refused at startup rather than resolved to `temporary` the way Python resolves it. (`multicast_address_type` (`ini_config.rs:646-652`); `InterfaceConfig::multicast_address_type` (`config.rs:550-555`); `MulticastAddressType` (`interfaces/auto_interface/mod.rs:42-91`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `device` | string | default adapter | BlueZ adapter to use, e.g. `hci0`. (`device` (`ini_config.rs:525`)) |
| `max_connections` | usize | `4` | Simultaneous BLE link cap, both GATT roles counted together. The default is the firmware's `MAX_LINKS` (4), not the reference's 7: 3-4 links is the protocol's reliable ceiling. (`max_connections` (`ini_config.rs:710`); `InterfaceConfig::max_connections` (`config.rs:669-672`)) |
| `min_rssi` | i16 (dBm) | `-85` | Sightings weaker than this are not dialled. (`min_rssi` (`ini_config.rs:711`); `InterfaceConfig::min_rssi` (`config.rs:673-675`)) |
...
| `speed` / `baudrate` | u32 | unset | Serial baud rate (either spelling). (`speed` (`ini_config.rs:541`); `InterfaceConfig::speed` (`config.rs:482-483`)) |
| `databits` | u8 | unset | Data bits. (`databits` (`ini_config.rs:542`); `InterfaceConfig::databits` (`config.rs:484-485`)) |
| `parity` | string | unset | `none`, `even`, or `odd`. (`parity` (`ini_config.rs:543`); `InterfaceConfig::parity` (`config.rs:486-487`)) |
| `stopbits` | u8 | unset | Stop bits. (`stopbits` (`ini_config.rs:544`); `InterfaceConfig::stopbits` (`config.rs:488-489`)) |
...
| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `networkname` / `network_name` | string | unset | Network name for IFAC (either spelling). (`networkname` (`ini_config.rs:689`); `InterfaceConfig::networkname` (`config.rs:598-600`)) |
| `passphrase` / `pass_phrase` | string | unset | IFAC passphrase (either spelling). (`passphrase` (`ini_config.rs:601`); `InterfaceConfig::passphrase` (`config.rs:600-601`)) |
| `ifac_size` | usize (bits) | unset | IFAC size, specified in bits in the file and stored as bytes (`bits / 8`). Values below 8 bits are dropped, as Python drops them, so the interface falls back to its per-type default. (`ifac_size` (`ini_config.rs:691-697`); `InterfaceConfig::ifac_size` (`config.rs:602-603`)) |
