...
| `BLE_CENTRAL_*`, `BLE_LINK_SELF`, `BLE_LINK_DUP` | `leviculum-nrf/src/ble/columba.rs` | Central-role connection lifecycle: `BLE_CENTRAL_ADDR`/`CONNECT`/`FAIL`/`UP`/`DOWN`, plus the two admission decisions.  `BLE_LINK_SELF addr=<a> action=disconnect` — the peer presented our own identity.  `BLE_LINK_DUP peer=<hex8> addr=<a> action=refuse\|displace old_conn=<h> new_conn=<h> old_age_ms=<n>` — that identity already holds a live link (#376); `old_age_ms` is how long ago the OLD link last carried a non-keepalive frame and is the whole decision: below 30 s (`ZOMBIE_TIMEOUT_MS`) the old link is alive and the newcomer is refused, at or above it the old link is a zombie and is displaced, with `moved=`/`dropped=` counting the packets carried over from its queue.  Emitted in BOTH roles, so a phone dialling in and a board dialling out read the same.  A refusal also emits `BLE_DIAL_DEAD_END addr=<a> reason=dup_refused ttl_s=<n>`: the address is backed off, or the scanner re-offers it within seconds.  Running totals are `refused=` and `displaced=` on `BLE_COUNTERS`; beside a Columba phone it is `refused=` that should climb. |
...
congested link is `BLE_TX_FANOUT_DROP`.  The delivery-hint decision
