...
| `BLE_CENTRAL_*`, `BLE_LINK_SELF`, `BLE_LINK_DUP` | `leviculum-nrf/src/ble/columba.rs` | Central-role connection lifecycle: `BLE_CENTRAL_ADDR`/`CONNECT`/`FAIL`/`UP`/`DOWN`, plus the two admission decisions.  `BLE_LINK_SELF addr=<a> action=disconnect` — the peer presented our own identity.  `BLE_LINK_DUP peer=<hex8> addr=<a> action=refuse\|displace origin=incoming\|outgoing old_conn=<h> new_conn=<h> old_silence_ms=<n>` — that identity already holds a live link (#376, #382).  `origin=` is the decision: an `incoming` duplicate (the peer dialled us) always displaces, because a peer that opens a second connection has by its own one-link rule given up on the first; an `outgoing` one (our own dial) is refused unless `old_silence_ms` has reached `LINK_TIMEOUT_MS` (45 s), meaning the old link has delivered nothing at all — payload AND keepalives.  `old_silence_ms` is how long ago the OLD link last delivered ANY frame, and it is reported on both branches even where it is not consulted.  A displacement adds `moved=`/`dropped=`, the packets carried over from the old link's queue.  Emitted in BOTH roles, so a phone dialling in and a board dialling out read the same.  A refusal also emits `BLE_DIAL_DEAD_END addr=<a> reason=dup_refused ttl_s=<n>`: the address is backed off, or the scanner re-offers it within seconds.  Running totals are `refused=` and `displaced=` on `BLE_COUNTERS`; beside a Columba phone both climb — `refused=` per fallback dial that finds the phone's rotated address, `displaced=` once per reconnect the phone itself initiates. |
