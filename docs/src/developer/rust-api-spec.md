...
| `fn default_config_dir() -> PathBuf` — `config.rs:990` | Default config directory |
| `fn default_config_path() -> PathBuf` — `config.rs:999` | Default config file path |
...
| `fn handle_packet(&mut self, iface: InterfaceId, data: &[u8]) -> TickOutput` — `node/mod.rs:2101` | Feed received bytes from an interface |
| `fn handle_timeout(&mut self) -> TickOutput` — `node/mod.rs:2334` | Run periodic maintenance (call at the next deadline) |
| `fn next_deadline(&self) -> Option<u64>` — `node/mod.rs:2365` | Earliest timer deadline (ms); when to call `handle_timeout` |
