...
`target_interface` (`transport.rs:9371`) — and the retry scheduler
hands an entry carrying one to that interface alone instead of
broadcasting it: `target_iface` (`transport.rs:10024-10048`).
...
| `LOCAL_REBROADCASTS_MAX` | 2 | 2 | ✓ | `LOCAL_REBROADCASTS_MAX` (`constants.rs:153`); enforced in the retry loop, `local_rebroadcasts` (`transport.rs:9855`), and on a duplicate arrival, `local_rebroadcasts` (`transport.rs:5212`) |
| `ANNOUNCE_CAP` | 2 % | 2 % | ✓ | `DEFAULT_ANNOUNCE_CAP_PERCENT` (`constants.rs:333`); state in `InterfaceAnnounceCap` (`transport.rs:601-608`), holdoff at `allowed_at_ms` (`transport.rs:10224-10236`) |
...
| Path-response targeted | targeted-transport branch, section 5 | `target_iface` (`transport.rs:10024-10048`) | ✓ | Preserved |
...
- the two guards are `PATHFINDER_RETRIES` (`transport.rs:9854`)
  and `local_rebroadcasts` (`transport.rs:9855`), in
