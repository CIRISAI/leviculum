...
`target_interface` (`transport.rs:9586`) — and the retry scheduler
hands an entry carrying one to that interface alone instead of
broadcasting it: `target_iface` (`transport.rs:10215-10239`).
...
4286 sits inside `announce_table_entries` (`transport.rs:4292`), an
...
|---|---|---|---|---|
| Self-announce one-shot | `Destination.py:322`, `Packet.py:294` | one emission per call: `send_on_all_interfaces` (`transport.rs:3384`) | ✓ | History: the 3 extra retries this row recorded were removed by B3, 08128e97 (2026-04-15) |
...
| Self-announce fanout | all interfaces (MODE_FULL assumed) | no exclusion argument | ✓ | `send_on_all_interfaces` (`transport.rs:3384`) |
| Received-announce rebroadcast count | 2 (non-local-client), 1 (local-client) | 2 and 1: the insert picks the start value from the source, `retries` (`transport.rs:5650-5654`) | ✓ | History: B2, 79ac5204 (2026-04-15), dropped `PATHFINDER_RETRIES` to 1 and reordered the init |
...
| Packet-hash dedup on RX | `Transport.py:1227` | `has_packet_hash` (`transport.rs:3094`) | ✓ | Identical semantics, rolling window |
...
| `LOCAL_REBROADCASTS_MAX` | 2 | 2 | ✓ | `LOCAL_REBROADCASTS_MAX` (`constants.rs:153`); enforced in the retry loop, `local_rebroadcasts` (`transport.rs:10070`), and on a duplicate arrival, `local_rebroadcasts` (`transport.rs:5358`) |
...
| Path-response targeted | targeted-transport branch, section 5 | `target_iface` (`transport.rs:10215-10239`) | ✓ | Preserved |
...
- the insert picks the start value from the source of the
  ...
  `0` otherwise — at `retries` (`transport.rs:5650-5654`);
- the two guards are `PATHFINDER_RETRIES` (`transport.rs:10069`)
  and `local_rebroadcasts` (`transport.rs:10070`), in
...
`send_on_all_interfaces` (`transport.rs:3384`), which calls
`self.storage.add_packet_hash()` before emitting the
`Action::Broadcast`. The check that reads that set on arrival is
`has_packet_hash` (`transport.rs:3094`), in `process_incoming`. The only edge case is the
...
by `send_on_all_interfaces` (`transport.rs:3384`) and then once more
by `schedule_own_announce_retry` (`transport.rs:3455`), which inserts
...
`schedule_own_announce_retry` (`transport.rs:3455`).
