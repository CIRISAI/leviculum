...
`target_interface` (`transport.rs:9501`) — and the retry scheduler
hands an entry carrying one to that interface alone instead of
broadcasting it: `target_iface` (`transport.rs:10154-10178`).
...
4286 sits inside `announce_table_entries` (`transport.rs:4282`), an
...
|---|---|---|---|---|
| Self-announce one-shot | `Destination.py:322`, `Packet.py:294` | one emission per call: `send_on_all_interfaces` (`transport.rs:3375`) | ✓ | History: the 3 extra retries this row recorded were removed by B3, 08128e97 (2026-04-15) |
...
| Self-announce fanout | all interfaces (MODE_FULL assumed) | no exclusion argument | ✓ | `send_on_all_interfaces` (`transport.rs:3375`) |
| Received-announce rebroadcast count | 2 (non-local-client), 1 (local-client) | 2 and 1: the insert picks the start value from the source, `retries` (`transport.rs:5616-5620`) | ✓ | History: B2, 79ac5204 (2026-04-15), dropped `PATHFINDER_RETRIES` to 1 and reordered the init |
...
| Packet-hash dedup on RX | `Transport.py:1227` | `has_packet_hash` (`transport.rs:3085`) | ✓ | Identical semantics, rolling window |
...
| Path-response targeted | targeted-transport branch, section 5 | `target_iface` (`transport.rs:10154-10178`) | ✓ | Preserved |
...
- the insert picks the start value from the source of the
  ...
  `0` otherwise — at `retries` (`transport.rs:5616-5620`);
- the two guards are `PATHFINDER_RETRIES` (`transport.rs:9984`)
  and `local_rebroadcasts` (`transport.rs:9985`), in
...
`send_on_all_interfaces` (`transport.rs:3375`), which calls
`self.storage.add_packet_hash()` before emitting the
`Action::Broadcast`. The check that reads that set on arrival is
`has_packet_hash` (`transport.rs:3085`), in `process_incoming`. The only edge case is the
...
by `send_on_all_interfaces` (`transport.rs:3375`) and then once more
by `schedule_own_announce_retry` (`transport.rs:3446`), which inserts
...
`schedule_own_announce_retry` (`transport.rs:3446`).
