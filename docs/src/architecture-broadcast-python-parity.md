...
the `target_interface: Some(idx)` branch at `transport.rs:4202-4152`.
...
|---|---|---|---|---|
| Self-announce one-shot | `Destination.py:322`, `Packet.py:294` | `transport.rs:1356-1329` schedules 3 retries beyond the initial | ✗ | **Fixed in B3** |
| Self-announce on-wire count | 1 | 4 (1 + 3 retries) | ✗ | **B3 brings to 1** |
| Self-announce fanout | all interfaces (MODE_FULL assumed) | `send_on_all_interfaces(exclude=None)` | ✓ | `transport.rs:1343-1303` |
...
| Packet-hash dedup on RX | `Transport.py:1227` | `transport.rs:1271` | ✓ | Identical semantics, rolling window |
...
| `LOCAL_REBROADCASTS_MAX` | 2 | 2 | ✓ | `constants.rs:133`; enforcement at `transport.rs:4092` |
| `ANNOUNCE_CAP` | 2 % | 2 % | ✓ | `constants.rs:242`; impl at `transport.rs:286-295, 4125` |
...
| Path-response targeted | `transport.rs:4202-4152` | same mechanism | ✓ | Preserved |
...
- A. Set `PATHFINDER_RETRIES = 1` **and** change the entry-insert
  at `transport.rs:2099-2035` from `retries: 1` to `retries: 0`.
  Guards at `transport.rs:4091-4027` already read
...
`send_on_all_interfaces` at `transport.rs:1343-1303` which calls
`self.storage.add_packet_hash()` before emitting the
`Action::Broadcast`. The dedup check at `transport.rs:1271`
