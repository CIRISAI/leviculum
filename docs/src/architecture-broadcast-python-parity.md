...
the `target_interface: Some(idx)` branch at `transport.rs:4143-4093`.
...
|---|---|---|---|---|
| Self-announce one-shot | `Destination.py:322`, `Packet.py:294` | `transport.rs:1328-1301` schedules 3 retries beyond the initial | ✗ | **Fixed in B3** |
| Self-announce on-wire count | 1 | 4 (1 + 3 retries) | ✗ | **B3 brings to 1** |
| Self-announce fanout | all interfaces (MODE_FULL assumed) | `send_on_all_interfaces(exclude=None)` | ✓ | `transport.rs:1315-1275` |
...
| Packet-hash dedup on RX | `Transport.py:1227` | `transport.rs:1243` | ✓ | Identical semantics, rolling window |
...
| `LOCAL_REBROADCASTS_MAX` | 2 | 2 | ✓ | `constants.rs:133`; enforcement at `transport.rs:4033` |
...
| Path-response targeted | `transport.rs:4143-4093` | same mechanism | ✓ | Preserved |
...
- A. Set `PATHFINDER_RETRIES = 1` **and** change the entry-insert
  at `transport.rs:2060-1996` from `retries: 1` to `retries: 0`.
  Guards at `transport.rs:4032-3968` already read
...
`send_on_all_interfaces` at `transport.rs:1315-1275` which calls
`self.storage.add_packet_hash()` before emitting the
`Action::Broadcast`. The dedup check at `transport.rs:1243`
