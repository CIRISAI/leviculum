...
`target_interface` (`transport.rs:9687`) — and the retry scheduler
hands an entry carrying one to that interface alone instead of
broadcasting it: `target_iface` (`transport.rs:10316-10340`).
...
4286 sits inside `announce_table_entries` (`transport.rs:4364`), an
...
|---|---|---|---|---|
| Self-announce one-shot | `Destination.py:322`, `Packet.py:294` | one emission per call: `send_on_all_interfaces` (`transport.rs:3456`) | ✓ | History: the 3 extra retries this row recorded were removed by B3, 08128e97 (2026-04-15) |
...
| Self-announce fanout | all interfaces (MODE_FULL assumed) | no exclusion argument | ✓ | `send_on_all_interfaces` (`transport.rs:3456`) |
...
| Path-response targeted | targeted-transport branch, section 5 | `target_iface` (`transport.rs:10316-10340`) | ✓ | Preserved |
...
- the insert picks the start value from the source of the
  ...
  `0` otherwise — at `retries` (`transport.rs:5722-5726`);
...
`send_on_all_interfaces` (`transport.rs:3456`), which calls
`self.storage.add_packet_hash()` before emitting the
`Action::Broadcast`. The check that reads that set on arrival is
`has_packet_hash` (`transport.rs:3166`), in `process_incoming`. The only edge case is the
...
by `send_on_all_interfaces` (`transport.rs:3456`) and then once more
