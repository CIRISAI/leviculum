...
node answers a path request from a network peer (`handle_path_request` case 2b, `transport.rs:6188`)
...
(`set_announce_cache` stores the raw pre-increment buffer; the receipt increment at `transport.rs:1707`
...
that a strict Python client accepts it (`transport.rs:3858`, commit `5d0833d7`). It buys
