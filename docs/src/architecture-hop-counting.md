...
node answers a path request from a network peer (`handle_path_request` case 2b, `transport.rs:6379`)
...
| Path-response hop emission | `:2997` (`packet.hops = path_table[dst][IDX_PT_HOPS]`), `:618` | `transport.rs:6379` (case 2b emits the stored path-table count) | matches — **fixed 2026-07-10 (D3, commit `path-response-hops`); previously emitted `cached_packet.hops` = the pre-increment wire byte (`stored - 1`)** |
...
| Healing, local client link (`taken_hops == 0`) | `:744` | `transport.rs:7048` | matches — **fixed 2026-07-10 (D1, commit `74ac655`); was absent** |
| Healing, destination direct | `:753` | `transport.rs:7048` | matches |
| Healing, initiator direct (`taken_hops == 1`) | `:775` | `transport.rs:7067` | matches |
