...
node answers a path request from a network peer (`handle_path_request` case 2b, `transport.rs:6837`)
...
(`set_announce_cache` stores the raw pre-increment buffer; the receipt increment at `transport.rs:2047`
...
|------|-----------|-----------|---------|
| Receipt increment | `:1498` | `transport.rs:1952` | matches |
...
| Path table store | `:1909`, `:2055` | `transport.rs:3518` | matches |
| Path acceptance | `:1806`, `:2412` | `transport.rs:3614` (`should_update`) | matches |
| Path-response hop emission | `:2997` (`packet.hops = path_table[dst][IDX_PT_HOPS]`), `:618` | `transport.rs:6837` (case 2b emits the stored path-table count) | matches — **fixed 2026-07-10 (D3, commit `path-response-hops`); previously emitted `cached_packet.hops` = the pre-increment wire byte (`stored - 1`)** |
| Link entry fields | `:1615-1625` | `storage_types.rs:60 (destination_hash at :76)` | matches, including the destination hash |
| LRPROOF relay check | `:2215-2206` (single `== remaining_hops`, drop else; the `:1697` disjunction is gated OUT for LRPROOF at `:1687`) | `transport.rs:4285`; rewritten by default, DROPPED behind `lrproof_rewrite_on_asymmetry=false` | **deliberate deviation** (default); the flagged strict branch drops like the reference, but see the mapping caveat below |
| Healing, no path | `:737` | `transport.rs:7270` | matches |
| Healing, local client link (`taken_hops == 0`) | `:744` | `transport.rs:7547` | matches — **fixed 2026-07-10 (D1, commit `74ac655`); was absent** |
| Healing, destination direct | `:753` | `transport.rs:7278` | matches |
| Healing, initiator direct (`taken_hops == 1`) | `:775` | `transport.rs:7566` | matches |
...
that a strict Python client accepts it (`transport.rs:4309`, commit `5d0833d7`). It buys
