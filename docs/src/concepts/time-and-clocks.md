...
`Transport::emission_secs` (`leviculum-core/src/transport.rs:3279`).
...
(`leviculum-core/src/node/mod.rs:2673`), which is correct only until
...
- **Unbounded first adoption.** `learn_emission_timebase`
  (`leviculum-core/src/transport.rs:3423`) selects the unbounded
...
- **The ticket refusal.** `NodeCore::has_plausible_wall_clock`
  (`leviculum-core/src/node/mod.rs:2673`) becomes vacuously true at
...
(`transport.rs:3423`) adopts the highest emission timestamp seen in
...
(`leviculum-core/src/transport.rs:3279`) — plus a bounded forward
...
(`transport.rs:18346`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:18420`). No durable defence is claimed from the
...
(`learn_emission_timebase`, `transport.rs:3423`) implements the
...
(`transport.rs:18265`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:18480`); arm 4's ceiling at
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:18420`). The arm-1 trio lands with GNSS seeding and
...
(`transport.rs:18162`) and
`test_clockless_timebase_advance_is_bounded_after_first_adoption`
(`transport.rs:18113`); the rank-keyed cells replace their
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:18072`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:18346`) and stays a negative cell: the assertion is
