...
(`transport.rs:18333`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:18407`). No durable defence is claimed from the
...
(`transport.rs:18252`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:18467`); arm 4's ceiling at
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:18407`). The arm-1 trio lands with GNSS seeding and
...
(`transport.rs:18149`) and
`test_clockless_timebase_advance_is_bounded_after_first_adoption`
(`transport.rs:18100`); the rank-keyed cells replace their
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:18059`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:18333`) and stays a negative cell: the assertion is
