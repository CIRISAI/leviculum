...
`Transport::emission_secs` (`leviculum-core/src/transport.rs:4173`).
...
- **Unbounded first adoption.** `learn_emission_timebase`
  (`leviculum-core/src/transport.rs:4360`) selected the unbounded
...
- **Status:** the implementation consults this arm first when it
  answers (`emission_secs`, `leviculum-core/src/transport.rs:4173`).
  ...
  `leviculum-core/src/transport.rs:4294`); below the build floor it
...
(`leviculum-core/src/transport.rs:4173`) — plus a bounded forward
...
`leviculum-core/src/transport.rs:4360`): a node starting at the
...
(`leviculum-core/src/transport.rs:21514`).
...
(`transport.rs:21744`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:21818`). No durable defence is claimed from the
...
(`learn_emission_timebase`, `transport.rs:4360`) implements the
...
(`leviculum-core/src/transport.rs:4294`) names the arm — including
...
(`announce_emission_secs`, `leviculum-core/src/transport.rs:4248`);
...
(`leviculum-core/src/transport.rs:21473`), which carries the
...
(`transport.rs:21416`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:21879`). Arm 3 is
`test_platform_clock_outside_the_window_is_not_a_time_source`
(`leviculum-core/src/transport.rs:21593`). Arm 4's floor is
`test_implausibly_low_floor_recovers_in_one_adoption`
(`leviculum-core/src/transport.rs:21261`) and its ceiling
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:21818`).
...
`leviculum-core/src/transport.rs:21550`) AND tickets are refused
...
`test_clockless_timebase_advance_is_bounded_after_first_adoption`,
`transport.rs:21161`); and the negative that catches the value-test
misfire is
`test_birth_anchor_value_clears_the_window_but_not_the_rank`
(`leviculum-core/src/transport.rs:21514`) — the build-floor value
alone never satisfies the plausibility predicate, even though it
clears the sanity window. The older
`test_clockless_first_timebase_adoption_is_unbounded`
(`transport.rs:21212`) keeps its assertions and now asserts the
...
(`leviculum-core/src/transport.rs:21645`) walks a sequence of
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:21120`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:21744`) and stays a negative cell: the assertion is
