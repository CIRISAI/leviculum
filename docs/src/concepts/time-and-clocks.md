...
(`leviculum-core/src/traits.rs:379`). All timeout and deadline
...
`Transport::emission_secs` (`leviculum-core/src/transport.rs:3879`).
...
- **Unbounded first adoption.** `learn_emission_timebase`
  (`leviculum-core/src/transport.rs:4066`) selected the unbounded
...
rank is `TimeSource::rank` (`leviculum-core/src/transport.rs:1254`)
and `BIRTH_ANCHOR_RANK` (`leviculum-core/src/transport.rs:1270`) is
...
> `NodeCore::emission_secs` (`leviculum-core/src/node/mod.rs:3372`) rustdoc
> listed the chain as
...
- **Guarantees:** host-clock quality, sanity-gated: values outside
  ...
  are refused (`leviculum-core/src/transport.rs:3995`), because an
  injection *claims*
...
`Clock::wall_unix_secs` (`leviculum-core/src/traits.rs:398`). On std
...
- **Guarantees:** whatever the platform clock guarantees — NTP
  ...
  arithmetic stays on the monotonic `now_ms` (`traits.rs:379`).
- **Status:** the implementation consults this arm first when it
  answers (`emission_secs`, `leviculum-core/src/transport.rs:3879`).
  ...
  `leviculum-core/src/transport.rs:4000`); below the build floor it
...
(`transport.rs:4066`) adopts the highest emission timestamp seen in
any signature-valid announce as the calendar anchor, then advances it
with the monotonic clock (`transport.rs:3453`). This includes the
...
(`leviculum-core/src/transport.rs:3879`) — plus a bounded forward
...
point of resolution (`transport.rs:3463`) and again at the wire
...
(`emitted_secs <= current`, `leviculum-core/src/transport.rs:4085`),
...
(`leviculum-core/src/transport.rs:4070`); the lower bound is the build floor
...
`leviculum-core/src/transport.rs:4066`): a node starting at the
...
(`leviculum-core/src/transport.rs:20581`).
...
(`transport.rs:20811`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:20885`). No durable defence is claimed from the
...
(`learn_emission_timebase`, `transport.rs:4066`) implements the
...
`NodeCore::anchor_rank` (`leviculum-core/src/node/mod.rs:3404`) is
...
(`announce_emission_secs`, `leviculum-core/src/transport.rs:3954`);
...
(`leviculum-core/src/transport.rs:20540`), which carries the
...
(`transport.rs:20483`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:20946`). Arm 3 is
`test_platform_clock_outside_the_window_is_not_a_time_source`
(`leviculum-core/src/transport.rs:20660`). Arm 4's floor is
`test_implausibly_low_floor_recovers_in_one_adoption`
(`leviculum-core/src/transport.rs:20328`) and its ceiling
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:20885`).
...
`leviculum-core/src/transport.rs:20617`) AND tickets are refused
...
`test_clockless_timebase_advance_is_bounded_after_first_adoption`,
`transport.rs:20228`); and the negative that catches the value-test
misfire is
`test_birth_anchor_value_clears_the_window_but_not_the_rank`
(`leviculum-core/src/transport.rs:20581`) — the build-floor value
alone never satisfies the plausibility predicate, even though it
clears the sanity window. The older
`test_clockless_first_timebase_adoption_is_unbounded`
(`transport.rs:20279`) keeps its assertions and now asserts the
...
(`leviculum-core/src/transport.rs:20712`) walks a sequence of
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:20187`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:20811`) and stays a negative cell: the assertion is
