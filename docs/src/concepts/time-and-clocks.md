...
`Transport::emission_secs` (`leviculum-core/src/transport.rs:3608`).
...
(`leviculum-core/src/node/mod.rs:3183`), which is correct only until
...
- **Unbounded first adoption.** `learn_emission_timebase`
  (`leviculum-core/src/transport.rs:3752`) selects the unbounded
...
- **The ticket refusal.** `NodeCore::has_plausible_wall_clock`
  (`leviculum-core/src/node/mod.rs:3183`) becomes vacuously true at
...
(`transport.rs:3752`) adopts the highest emission timestamp seen in
...
(`leviculum-core/src/transport.rs:3608`) — plus a bounded forward
...
(`transport.rs:19123`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:19197`). No durable defence is claimed from the
...
(`learn_emission_timebase`, `transport.rs:3752`) implements the
...
(`transport.rs:19042`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:19257`); arm 4's ceiling at
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:19197`). The arm-1 trio lands with GNSS seeding and
...
(`transport.rs:18939`) and
`test_clockless_timebase_advance_is_bounded_after_first_adoption`
(`transport.rs:18890`); the rank-keyed cells replace their
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:18849`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:19123`) and stays a negative cell: the assertion is
