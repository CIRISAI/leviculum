...
`Transport::emission_secs` (`leviculum-core/src/transport.rs:3024`).
...
- **Unbounded first adoption.** `learn_emission_timebase`
  (`leviculum-core/src/transport.rs:3168`) selects the unbounded
...
(`transport.rs:3168`) adopts the highest emission timestamp seen in
...
(`leviculum-core/src/transport.rs:3024`) — plus a bounded forward
...
(`transport.rs:17789`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:17863`). No durable defence is claimed from the
...
(`transport.rs:17708`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:17923`); arm 4's ceiling at
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:17863`). The arm-1 trio lands with GNSS seeding and
...
(`transport.rs:17605`) and
`test_clockless_timebase_advance_is_bounded_after_first_adoption`
(`transport.rs:17556`); the rank-keyed cells replace their
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:17515`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:17789`) and stays a negative cell: the assertion is
