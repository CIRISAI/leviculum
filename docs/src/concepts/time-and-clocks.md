...
(`leviculum-core/src/transport.rs:20720`).
...
(`transport.rs:20950`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:21024`). No durable defence is claimed from the
...
(`leviculum-core/src/transport.rs:20679`), which carries the
...
(`transport.rs:20622`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:21085`). Arm 3 is
`test_platform_clock_outside_the_window_is_not_a_time_source`
(`leviculum-core/src/transport.rs:20799`). Arm 4's floor is
`test_implausibly_low_floor_recovers_in_one_adoption`
(`leviculum-core/src/transport.rs:20467`) and its ceiling
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:21024`).
...
`leviculum-core/src/transport.rs:20756`) AND tickets are refused
...
`test_clockless_timebase_advance_is_bounded_after_first_adoption`,
`transport.rs:20367`); and the negative that catches the value-test
misfire is
`test_birth_anchor_value_clears_the_window_but_not_the_rank`
(`leviculum-core/src/transport.rs:20720`) — the build-floor value
alone never satisfies the plausibility predicate, even though it
clears the sanity window. The older
`test_clockless_first_timebase_adoption_is_unbounded`
(`transport.rs:20418`) keeps its assertions and now asserts the
...
(`leviculum-core/src/transport.rs:20851`) walks a sequence of
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:20326`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:20950`) and stays a negative cell: the assertion is
