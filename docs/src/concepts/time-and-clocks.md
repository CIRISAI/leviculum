...
`Transport::emission_secs` (`leviculum-core/src/transport.rs:3167`).
...
- **Unbounded first adoption.** `learn_emission_timebase`
  (`leviculum-core/src/transport.rs:3311`) selects the unbounded
  branch today by `current < EMISSION_PLAUSIBLE_MIN_SECS`
  (`transport.rs:3018`). A birth-anchored cold node clears that
...
> `transport.rs:2964`) says a platform wall clock always takes
...
→ `transport.rs:2964`), for deployments where a clockless node has a
...
- **Guarantees:** host-clock quality, sanity-gated: values outside
  ...
  are refused (`transport.rs:2972`), because an injection *claims*
...
- **Status:** the implementation consults this arm first when it
  answers (`transport.rs:2869`). No current platform offers both a
...
(`transport.rs:3311`) adopts the highest emission timestamp seen in
any signature-valid announce as the calendar anchor, then advances it
with the monotonic clock (`transport.rs:2874`). This includes the
...
- **Status:** spec. Today an instance with no source emits raw
  uptime seconds (`transport.rs:2876`), which every cross-restart
...
(`leviculum-core/src/transport.rs:3167`) — plus a bounded forward
...
point of resolution (`transport.rs:2884`) and again at the wire
...
(`emitted_secs <= current`, `transport.rs:3007`), and adoption is
...
(`transport.rs:3003`); the lower bound `EMISSION_PLAUSIBLE_MIN_SECS`
...
deliberately unbounded (`transport.rs:3018`): a node starting at the
...
(`transport.rs:17932`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:18006`). No durable defence is claimed from the
...
(`transport.rs:2946`); exposing the current source and its origin
...
(`transport.rs:17851`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:18066`); arm 4's ceiling at
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:18006`). The arm-1 trio lands with GNSS seeding and
...
(`transport.rs:17748`) and
`test_clockless_timebase_advance_is_bounded_after_first_adoption`
(`transport.rs:17699`); the rank-keyed cells replace their
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:17658`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:17932`) and stays a negative cell: the assertion is
