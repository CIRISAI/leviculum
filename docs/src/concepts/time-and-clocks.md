...
`Transport::emission_secs` (`leviculum-core/src/transport.rs:3494`).
...
(`leviculum-core/src/node/mod.rs:2961`), which is correct only until
...
- **Unbounded first adoption.** `learn_emission_timebase`
  (`leviculum-core/src/transport.rs:3638`) selects the unbounded
  branch today by `current < EMISSION_PLAUSIBLE_MIN_SECS`
  (`transport.rs:3394`). A birth-anchored cold node clears that
...
- **The ticket refusal.** `NodeCore::has_plausible_wall_clock`
  (`leviculum-core/src/node/mod.rs:2961`) becomes vacuously true at
...
> `transport.rs:3331`) says a platform wall clock always takes
...
`Node::set_wall_time_unix_secs` (`leviculum-core/src/node/mod.rs:805`
→ `transport.rs:3331`), for deployments where a clockless node has a
...
- **Guarantees:** host-clock quality, sanity-gated: values outside
  ...
  are refused (`transport.rs:3340`), because an injection *claims*
...
- **Status:** the implementation consults this arm first when it
  answers (`transport.rs:3232`). No current platform offers both a
...
(`transport.rs:3638`) adopts the highest emission timestamp seen in
any signature-valid announce as the calendar anchor, then advances it
with the monotonic clock (`transport.rs:3237`). This includes the
...
- **Status:** spec. Today an instance with no source emits raw
  uptime seconds (`transport.rs:3239`), which every cross-restart
...
(`leviculum-core/src/transport.rs:3494`) — plus a bounded forward
...
point of resolution (`transport.rs:3247`) and again at the wire
...
(`emitted_secs <= current`, `transport.rs:3383`), and adoption is
...
(`transport.rs:3379`); the lower bound `EMISSION_PLAUSIBLE_MIN_SECS`
...
deliberately unbounded (`transport.rs:3394`): a node starting at the
...
(`transport.rs:18934`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:19008`). No durable defence is claimed from the
...
(`learn_emission_timebase`, `transport.rs:3638`) implements the
...
(`transport.rs:3305`); exposing the current source and its origin
...
(`transport.rs:18853`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:19068`); arm 4's ceiling at
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:19008`). The arm-1 trio lands with GNSS seeding and
...
(`transport.rs:18750`) and
`test_clockless_timebase_advance_is_bounded_after_first_adoption`
(`transport.rs:18701`); the rank-keyed cells replace their
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:18660`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:18934`) and stays a negative cell: the assertion is
