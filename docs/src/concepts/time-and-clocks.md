...
`Transport::emission_secs` (`leviculum-core/src/transport.rs:2982`).
...
- **Unbounded first adoption.** `learn_emission_timebase`
  (`leviculum-core/src/transport.rs:3126`) selects the unbounded
...
(`transport.rs:3126`) adopts the highest emission timestamp seen in
...
(`leviculum-core/src/transport.rs:2982`) — plus a bounded forward
...
(`transport.rs:17542`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:17616`). No durable defence is claimed from the
...
(`transport.rs:17461`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:17676`); arm 4's ceiling at
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:17616`). The arm-1 trio lands with GNSS seeding and
...
(`transport.rs:17358`) and
`test_clockless_timebase_advance_is_bounded_after_first_adoption`
(`transport.rs:17309`); the rank-keyed cells replace their
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:17268`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:17542`) and stays a negative cell: the assertion is
