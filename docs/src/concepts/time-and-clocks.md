...
`Transport::emission_secs` (`leviculum-core/src/transport.rs:3431`).
...
- **Unbounded first adoption.** `learn_emission_timebase`
  (`leviculum-core/src/transport.rs:3575`) selects the unbounded
...
(`transport.rs:3575`) adopts the highest emission timestamp seen in
...
(`leviculum-core/src/transport.rs:3431`) — plus a bounded forward
...
(`transport.rs:18817`); the walk terminates at the learn ceiling,
pinned at `test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:18891`). No durable defence is claimed from the
...
(`learn_emission_timebase`, `transport.rs:3575`) implements the
...
(`transport.rs:18736`) and
`test_absurd_wall_time_injection_is_refused`
(`transport.rs:18951`); arm 4's ceiling at
`test_timebase_floor_cannot_pass_learn_ceiling`
(`transport.rs:18891`). The arm-1 trio lands with GNSS seeding and
...
(`transport.rs:18633`) and
`test_clockless_timebase_advance_is_bounded_after_first_adoption`
(`transport.rs:18584`); the rank-keyed cells replace their
...
`test_own_announce_echo_reseeds_timebase_before_echo_drop`
(`transport.rs:18543`). The announce-timebase consequence — a peer
...
`test_timebase_walk_is_capped_per_announce_not_per_identity`
(`transport.rs:18817`) and stays a negative cell: the assertion is
