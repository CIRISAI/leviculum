...
- **`Clock`** (`traits.rs:419`) — supplies the monotonic `now_ms()`
  ...
  (`leviculum-std/src/clock.rs:45`), on the nRF52 it is the Embassy
...
- **`Storage`** (`traits.rs:500`) — supplies persistence and lookup
  ...
  no-op (`traits.rs:840`) so a RAM-only backend needs to implement
  nothing extra.
...
(`leviculum-core/src/node/mod.rs:377`,
