...
(`leviculum-std/src/driver/mod.rs:4515`) to cull the routing entries,
...
leaves (`remove_spawned`, `leviculum-std/src/driver/mod.rs:4030`).
...
- Every discovered AutoInterface peer becomes a separate handle
  ...
  `leviculum-std/src/interfaces/auto_interface/orchestrator.rs:168`;
  the per-peer handle at
  `InterfaceHandle`,
  `leviculum-std/src/interfaces/auto_interface/orchestrator.rs:805`).
...
(`plan_tx_to`, `leviculum-std/src/interfaces/ble/links.rs:677`, driven
from `send_packet`,
`leviculum-std/src/interfaces/ble/mod.rs:864`). Since Codeberg #376 the
...
`leviculum-nrf/src/bin/t114.rs:1101`;
`handle_interface_peer_lost`, `leviculum-nrf/src/bin/t114.rs:1129`;
`handle_interface_peer_up`, `leviculum-nrf/src/bin/t114.rs:1141`; the
same three in `handle_packet_from_peer`,
`leviculum-nrf/src/bin/rak4631.rs:1094`), the core stamps the peer onto
the path entry it installs, and a peer loss culls exactly the paths
through it (`drop_paths_via_peer`,
`leviculum-core/src/transport.rs:4593`). So the identity-shaped
...
`leviculum-std/src/interfaces/ble/bluez.rs:290`), and the orchestrator
restarts the strict scan phase on that event (`CentralGone`,
`leviculum-std/src/interfaces/ble/mod.rs:694`, into `note_reset`,
`leviculum-std/src/interfaces/ble/links.rs:1222`). The firmware restarts
...
(`plan_tx_to`, `leviculum-std/src/interfaces/ble/links.rs:677`).
...
(`interface_held_announces`, `leviculum-core/src/transport.rs:2381`, is
...
(`interface_names`, `leviculum-core/src/transport.rs:10718`), each
...
| Announce cap and egress slot (`interface_announce_caps`, `leviculum-core/src/transport.rs:2223`; `interface_next_slot_ms`, `leviculum-core/src/transport.rs:2418`) | **medium** — it rations a shared resource | parent (splitting it per link multiplies the budget by the link count) |
| Max-airtime backchannel (`interface_max_airtime_ms`, `leviculum-core/src/transport.rs:2444`) | **medium** | parent |
...
| BLE inter-packet gap (`LinkPacer`, `leviculum-std/src/interfaces/ble/links.rs:1285`) | **link**, except on the shared notify pipe where one pacer serves every subscriber (`leviculum-std/src/interfaces/ble/mod.rs:317`) | child, mostly |
...
(`plan_tx_to`, `leviculum-std/src/interfaces/ble/links.rs:677`), and a
peer loss is reported only when the *last* link for that identity dies
(`knows_identity`, `leviculum-std/src/interfaces/ble/links.rs:393`).
