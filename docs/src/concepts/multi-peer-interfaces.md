...
(`registry`, `leviculum-std/src/driver/mod.rs:4462`).
...
from `send_packet`,
`leviculum-std/src/interfaces/ble/mod.rs:832`). Since Codeberg #376 the
...
`leviculum-core/src/transport.rs:4144`). So the identity-shaped
...
`leviculum-std/src/interfaces/ble/mod.rs:662`, into `note_reset`,
`leviculum-std/src/interfaces/ble/links.rs:1139`). The firmware restarts
...
(`interface_announce_caps`, `leviculum-core/src/transport.rs:1860`
through `own_tunnel_ids`,
`leviculum-core/src/transport.rs:2100` — the `BTreeMap<usize, _>` and
...
(`interface_held_announces`, `leviculum-core/src/transport.rs:2004`, is
...
(`interface_names`, `leviculum-core/src/transport.rs:9850`), each
...
| Announce cap and egress slot (`interface_announce_caps`, `leviculum-core/src/transport.rs:1860`; `interface_next_slot_ms`, `leviculum-core/src/transport.rs:2041`) | **medium** — it rations a shared resource | parent (splitting it per link multiplies the budget by the link count) |
| Max-airtime backchannel (`interface_max_airtime_ms`, `leviculum-core/src/transport.rs:2049`) | **medium** | parent |
| Advertising and scanning (`reconcile_advertising`, `leviculum-std/src/interfaces/ble/mod.rs:778`; `ScanScheduler`, `leviculum-std/src/interfaces/ble/links.rs:1095`) | **medium** — one adapter | parent |
| IFAC | **medium** — it is a property of the configured section | parent |
| BLE inter-packet gap (`LinkPacer`, `leviculum-std/src/interfaces/ble/links.rs:1202`) | **link**, except on the shared notify pipe where one pacer serves every subscriber (`leviculum-std/src/interfaces/ble/mod.rs:307`) | child, mostly |
