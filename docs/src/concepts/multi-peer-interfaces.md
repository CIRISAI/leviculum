...
`leviculum-nrf/src/bin/t114.rs:987`;
`handle_interface_peer_lost`, `leviculum-nrf/src/bin/t114.rs:1015`;
`handle_interface_peer_up`, `leviculum-nrf/src/bin/t114.rs:1027`; the
same three in `handle_packet_from_peer`,
`leviculum-nrf/src/bin/rak4631.rs:967`), the core stamps the peer onto
the path entry it installs, and a peer loss culls exactly the paths
through it (`drop_paths_via_peer`,
`leviculum-core/src/transport.rs:3983`). So the identity-shaped
...
(`interface_announce_caps`, `leviculum-core/src/transport.rs:1774`
through `own_tunnel_ids`,
`leviculum-core/src/transport.rs:2014` — the `BTreeMap<usize, _>` and
...
(`interface_held_announces`, `leviculum-core/src/transport.rs:1918`, is
...
|---|---|---|
| Heap, 96 KiB pool (`HEAP_SIZE`, `leviculum-nrf/src/lib.rs:242`) | worst watermark 65 044 B of 98 304 (`rig-run/proof-372-t114.log`, 2026-09-08); typical 56 000-57 000 | 33 260 B at the worst point |
...
(`interface_names`, `leviculum-core/src/transport.rs:9561`), each
...
| Announce cap and egress slot (`interface_announce_caps`, `leviculum-core/src/transport.rs:1774`; `interface_next_slot_ms`, `leviculum-core/src/transport.rs:1955`) | **medium** — it rations a shared resource | parent (splitting it per link multiplies the budget by the link count) |
| Max-airtime backchannel (`interface_max_airtime_ms`, `leviculum-core/src/transport.rs:1963`) | **medium** | parent |
