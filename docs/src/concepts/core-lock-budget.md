...
(`leviculum-std/src/driver/mod.rs:3557`).
...
`dispatch_output` (`leviculum-std/src/driver/mod.rs:5191`) routes
...
(`leviculum-std/src/driver/mod.rs:3050`) is
...
`leviculum-std/src/driver/mod.rs:4364`). A `write(2)` to a USB disk
...
`leviculum-std/src/event_log.rs:1211`): the emitting thread does a
...
| event | where | says |
| --- | --- | --- |
| `ANN_SLOW` | `handle_announce` (`leviculum-core/src/transport.rs:5263`) | announce handling itself took ≥ 100 ms |
| `CORE_STALL` | `spawn_core_stall_watchdog` (`leviculum-std/src/driver/mod.rs:4191`) | an outside thread waited ≥ 250 ms for the core lock |
