...
`has_packet_hash` (`leviculum-core/src/transport.rs:2957`) drops a
repeated copy, `add_packet_hash`
(`leviculum-core/src/transport.rs:2998`) records it.

The forwarding decision lives in the media-agnostic core
(`forward_on_interface_from`,
`leviculum-core/src/transport.rs:6824`). Whether the relayed echo
