...
`has_packet_hash` (`leviculum-core/src/transport.rs:2623`) drops a
repeated copy, `add_packet_hash`
(`leviculum-core/src/transport.rs:2664`) records it.

The forwarding decision lives in the media-agnostic core
(`forward_on_interface_from`,
`leviculum-core/src/transport.rs:6216`). Whether the relayed echo
