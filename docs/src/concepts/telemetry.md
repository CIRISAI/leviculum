...
- **The target's public key, before anything else.** Encrypting to a
  ...
  `send_on_all_interfaces`, `leviculum-core/src/transport.rs:2818`)
...
- **A path, or a request for one.** `send_to_destination`
  (`leviculum-core/src/transport.rs:2923`) fails without a path entry.
  The primitive for obtaining one is `request_path`
  (`leviculum-core/src/node/mod.rs:2594`); a node with the key but no
