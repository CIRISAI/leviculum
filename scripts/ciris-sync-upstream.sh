...
cargo build -p reticulum-core -p reticulum-std
...
cargo fmt --all -- --check
cargo clippy -p reticulum-core -p reticulum-std -- -D warnings
echo "==> SYNC OK on '$SERIES_BRANCH'. Review the rebased series, then push."
echo "    (full interop suite additionally needs: pip install rns)"
